...
Este projeto consiste em um jogo de simulação tática casual desenvolvido com foco no ensino e exploração prática de **Otimização Combinatória** e **Teoria dos Grafos**. A aplicação simula uma sala de situação tática (_war room_) onde o usuário assume o papel de um comandante militar encarregado de implantar bases e estabelecer rotas logísticas seguras entre elas.
...
- **Restrições:**
    ...
    3. $\text{Distância}(i, j) \le \text{Alcance Máximo da Aresta}$ (restrição de conexão do terreno).
...
- **Arquitetura de Software:**
    ...
    - _Pattern Singleton:_ Implementado em `AudioManager.gd` para gerenciamento global de áudio e efeitos sonoros (_Game Juice_).
        
    - _Filtragem de Entrada (`_unhandled_input`):_ Desacoplamento entre os eventos de clique no mundo do jogo e interações na interface (`CanvasLayer`).
...
1. Acesse a aba **Releases** na barra lateral do repositório.
...
|**Entrada de Controle**|**Ação no Simulador**|
|---|---|
|**Clique Esquerdo (Mouse)**|Instanciar uma nova base no mapa / Interagir com os Sliders da UI|
|**Clique Direito (Mouse)**|Deletar uma base existente ao clicar sobre ela|
|**Scroll do Mouse (Wheel)**|Zoom in e Zoom out na câmera do mapa|
|**Arrasto na UI**|Ajustar os limites de nós, raio de aresta e conexões máximas|
...
As principais dificuldades encontradas envolveram a sincronização entre a camada de renderização da Godot e a estrutura de dados abstrata do grafo (lista de adjacência), além do isolamento correto de inputs da UI para evitar comandos acidentais no mapa.
